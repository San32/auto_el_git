...
sys.path.append('/home/comm/conda_work/cv2/auto_el_git/log')
import log
import torch
...
class VideoThread(QThread):
    ...
    #                                 w = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    #                                 h = int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    #                                 cv2.putText(frame, str,(10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
    #                                 cv2.putText(frame, f'width:{w} height:{h}',(10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
    #                                 self.change_pixmap_signal.emit(frame)
    #                                 if self._save_flag:
    #                                     cv2.imwrite('./save_image.jpg', frame)
    #                                     self.thread_state_signal.emit('save_image')
    ...
    #                         else:
    #                             QTest.qWait(1000)
    #                             self.logger.info(f'no play : {self.vi} : {count}')
    #                             self.change_pixmap_signal.emit(self.default_cv_img)
    #                     else:
    #                         self.logger.info(f'cap.isOpened() : faile {self.vi}...')
    #                         break
    #             except:
...
class CamPanel(QWidget):
    ...
    def initUI(self):
        ...
        self.btn_loadmodel = QPushButton("loadmodel")
        self.btn_loadmodel.clicked.connect(self.loadmodel)
        self.btn_detect = QPushButton("no detect")
